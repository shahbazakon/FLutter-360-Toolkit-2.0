...
Future<bool> _isValidServer(String url) async {
  try {
    ...
    Response _serverResponse = await get(Uri.parse("$url/test/dart"));
...
bool _convertData(Response _response, String recipientMail) {
  try {
...
class EmailAuth {
  ...
  Future<bool> config(Map<String, String> data) async {
    try {
      ...
      if (data.containsKey('server') && data.containsKey('serverKey') && data['server'] != null && data['server']!.length > 0 && data['serverKey'] != null && data['serverKey']!.length > 0) {
        ...
        if (await _isValidServer(data['server']!)) {
          ...
          throw "email-auth >> The remote server is not a valid.\nemail-auth >> configured server : \"${data['server']}\"";
        }
      } else {
        throw "email-auth >> Remote server configurations are not valid";
      }
    } catch (error) {
      print("\n--- package Error ---\n");
      print(error);
  ...
  Future<bool> sendOtp({required String recipientMail, int otpLength = 6}) async {
    try {
      ...
      if (this._server.isEmpty) {
        ...
        Response _response = await get(Uri.parse(
        ...
        Response _response = await get(Uri.parse("${this._server}/dart/auth/$recipientMail?CompanyName=${this.sessionName}&key=${this._serverKey}&otpLength=$otpLength"));
        return _convertData(_response, recipientMail);
      }
      return false;
    } catch (error) {
